...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="skpd_id">{{ trans('cruds.user.fields.skpd') }}</label>
                <select class="form-control select2 {{ $errors->has('skpd') ? 'is-invalid' : '' }}" name="skpd_id" id="skpd_id" required>
                    @foreach($skpds as $id => $skpd)
                        <option value="{{ $id }}" {{ ($user->skpd ? $user->skpd->id : old('skpd_id')) == $id ? 'selected' : '' }}>{{ $skpd }}</option>
                    @endforeach
                </select>
                @if($errors->has('skpd_id'))
                    <span class="text-danger">{{ $errors->first('skpd_id') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.user.fields.skpd_helper') }}</span>
            </div>
            <div class="form-group">
