...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.skpd.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.skpds.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="nm_skpd">{{ trans('cruds.skpd.fields.nm_skpd') }}</label>
                <input class="form-control {{ $errors->has('nm_skpd') ? 'is-invalid' : '' }}" type="text" name="nm_skpd" id="nm_skpd" value="{{ old('nm_skpd', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('is_sub_unit') ? 'is-invalid' : '' }}" name="is_sub_unit" id="is_sub_unit" required>
                    ...
                    @foreach(App\Skpd::IS_SUB_UNIT_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('is_sub_unit', '0') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sub_unit') ? 'is-invalid' : '' }}" type="number" name="sub_unit" id="sub_unit" value="{{ old('sub_unit') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('is_aktif') ? 'is-invalid' : '' }}" name="is_aktif" id="is_aktif" required>
                    ...
                    @foreach(App\Skpd::IS_AKTIF_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('is_aktif', '1') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
